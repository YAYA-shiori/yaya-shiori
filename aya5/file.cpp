 ...
 * -----------------------------------------------------------------------
 */
int	CFile::OpenXML(const yaya::string_t &name)
{
	for(std::list<CFile1>::iterator it = filelist.begin(); it != filelist.end(); it++)
		if (!name.compare(it->GetName())) {
			int	result = it->OpenXML();
			return result;
		}
...
/* -----------------------------------------------------------------------
 *  関数名  ：  CFile::ReadXML
 *  機能概要：  ファイルからXMLデータを読み取ります
 *
 *  返値　　：　0/1=失敗/成功
 * -----------------------------------------------------------------------
 */
int	CFile::ReadXML(const yaya::string_t &name, yaya::string_t &ostr, const yaya::string_t &xpath)
{
	for(std::list<CFile1>::iterator it = filelist.begin(); it != filelist.end(); it++)
		if (!name.compare(it->GetName())) {
			int	result = it->ReadXML(ostr,xpath);
			return result;
		}
